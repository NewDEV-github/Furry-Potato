...
public partial class Dialogue : Control {
    ...
    /// <summary>
    ///     Label that displays character name in dialogues
    /// </summary>
    ...
    public override void _Ready() {
        Hide();
        _dialogueText = GetNode<RichTextLabel>("Panel/PanelContainer/Text");
        _dialogueCharacterName = GetNode<Label>("Panel/PanelContainer/CharacterName");
    ...
    public void HandleChoice(int choice) {
        if (_currentDialogueLine.Name == "R1S1_MAIN") EndDialogue();
    }
    ...
    private async void DisplayLineByLine(DialogueLine dialogueLine) {
        ...
        foreach (var (lineId, text) in lines) {
            _dialogueText.Show();
            var characterName = "";
            ...
            if (characterName == "CHOICE" || text == "CHOICE") {
                ...
                _dialogueCharacterName.Text = characterName;
            }

            await ToSignal(this, "EnterPressed");
            if (characterName == "END" || text == "END") EndDialogue();
        }
    }
